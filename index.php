...
<?php
	...
	if (!empty($player) && !empty($units)) //check if a complete query has been submitted
	{
		if (file_exists("$mcroot/$world/stats/$playeruuid.json")) //test if we have stats for that player.
		{
			$stats = file_get_contents("$mcroot/$world/stats/$playeruuid.json"); //read stats file contents from uuid.json
		}
		elseif (file_exists("$mcroot/$world/stats/$player.json")) #workonthis//this needs a function to locate files without case-sensitivity
		{
			$stats = file_get_contents("$mcroot/$world/stats/$player.json"); //read stats file contents from player.json (legacy support)
		}
		else
		{
			$stats = null;
			echo "Player $player not found on this server.";
		}
		if (!empty($stats))
		{
